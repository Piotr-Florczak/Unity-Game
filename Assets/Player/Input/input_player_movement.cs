...
{
    // Ustaw isOnCooldown na true, aby zablokować kolejne ataki
    isOnCooldown = true;

    // Wywołaj funkcję attack_checker() na początku cooldown
    attack_checker();

    // Poczekaj na zakończenie bieżącej animacji
    yield return new WaitForSeconds(animacja.GetCurrentAnimatorStateInfo(0).length * 0.3f);

    // Ustaw isOnCooldown na false, aby odblokować ataki
    isOnCooldown = false;
}
